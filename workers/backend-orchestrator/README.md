...
`PANEL_URL` is the Vercel origin only, for example `https://compressor.example.com` (no `/api/orchestrator` suffix). `ORCHESTRATOR_SECRET` must match the value saved in Dashboard > Settings. `wrangler.jsonc` is the custom build/deployment configuration: it uses `src/index.ts`, the `video-pipeline-orchestrator` Worker name, a one-minute cron, and a 10 ms CPU limit.
