...
It runs the processor every 15 minutes and can also be started manually. Runs may overlap to provide multiple GitHub-hosted processors; database claims and leases prevent duplicate job ownership. Each run installs FFmpeg/FFprobe with the `libx264` encoder, polls continuously for up to six hours, and exits ten minutes early so a job in progress can be stopped and requeued safely. It does not deploy the panel or Cloudflare Worker.
